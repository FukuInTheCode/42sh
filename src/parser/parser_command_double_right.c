...
static int set_next(parser_t *parser, command_t *right)
{
    command_t *next = NULL;
    ...
    command_add(right, next);
    return 0;
}

command_t *parser_command_double_right(parser_t *parser)
{
    command_t *ret = NULL;
    char **argv = NULL;
    ...
    if (!ret)
        return NULL;
    command_set_type(ret, DOUBLE_RIGHT_RED);
